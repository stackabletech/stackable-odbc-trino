...
//! rather than being used as a normal Rust dependency. All the ODBC C ABI
//! entry points are generated by [`stackable_odbc_core::forward_ffi!`] from
//! the [`TrinoBackend`] implementation; the Driver Manager translates ANSI
//! calls, so only the Unicode (`W`) functions are exported.
//!
...
//! [`stackable_odbc_core::backend::Backend`] trait is synchronous. The
//! connection string is parsed in `backend::types::connect_params`, whose
//! `PARAM_*` constants are the authoritative key list.
...
mod tests {
    ...
    /// that table is a keyword no Windows user can set through the dialog, and
    /// invisibly so, because the dialog still opens and still writes a working
